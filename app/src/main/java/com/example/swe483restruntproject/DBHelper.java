...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase MyDB) {
        ...
        MyDB.execSQL("CREATE TABLE restaurants (id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT,descrption TEXT,speiclty TEXT,logo TEXT,latitude TEXT,longitude TEXT)");
        ...
        MyDB.execSQL("CREATE TABLE photos (id INTEGER PRIMARY KEY AUTOINCREMENT,photoUrl TEXT,restaurantid INTEGER)");
    }
    ...
    public boolean insertRestaurants(String name , String descrption,String speiclty,String logo ,String latitude ,String longitude){
        SQLiteDatabase MyDB = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
...
//        Cursor cursor =   MyDB.query("users",new String[]{"id","username","password","adminOf"},"username = '"+name+"'",null,null,null,null);
    ...
    public Cursor getAllRestAverageASC(){
        SQLiteDatabase MyDB = this.getWritableDatabase();
        Cursor cursor = MyDB.rawQuery("select * from restaurants" ,null);
        if(cursor.getCount() > 0)
            return cursor;
        return null;
    }
    ...
    public Cursor getAllRestArabian(){
        SQLiteDatabase MyDB = this.getWritableDatabase();
        Cursor cursor = MyDB.rawQuery("select * from restaurants where speiclty = 'Arabic'" ,new String[]{});
        if(cursor.getCount() > 0)
            return cursor;
        return null;
    }
    public Cursor getAllRestBurgers(){
        SQLiteDatabase MyDB = this.getWritableDatabase();
        Cursor cursor = MyDB.rawQuery("select * from restaurants where speiclty = 'burgers'" ,new String[]{});
        if(cursor.getCount() > 0)
            return cursor;
        return null;
    }
    public Cursor getAllRestPizza(){
        SQLiteDatabase MyDB = this.getWritableDatabase();
        Cursor cursor = MyDB.rawQuery("select * from restaurants where speiclty = 'pizza'" ,new String[]{});
        if(cursor.getCount() > 0)
            return cursor;
        return null;
    }
    public Cursor getRestByName(String name){
        ...
        Cursor cursor =   MyDB.query("restaurants",new String[]{"id","name","descrption","speiclty","logo","latitude","longitude"}
